...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        res = await client.post(f"{BASE}/auth/login", json={"email": "kunal@test1.com", "password": "kunal1234"})
        ...
        print("\n=== 2. Signup 3 players ===")
        for p in PLAYERS:
            res = await client.post(f"{BASE}/auth/signup", json={"username": p["username"], "email": p["email"], "password": PASSWORD})
            if res.status_code in (200, 201):
                ...
                res2 = await client.post(f"{BASE}/auth/login", json={"email": p["email"], "password": PASSWORD})
        ...
        # === 3. Create room ===
        print("\n=== 3. Create room ===")
        res = await client.post(f"{BASE}/rooms", json={"max_players": 8, "settings": {}}, headers={"Authorization": f"Bearer {HOST_TOKEN}"})
        ...
        print("\n=== 4. Join room ===")
        for p in PLAYERS:
            res = await client.post(f"{BASE}/rooms/join", json={"code": room_code}, headers={"Authorization": f"Bearer {player_tokens[p['email']]}"})
        ...
        # === 6. Start game ===
        print("\n=== 6. Start game ===")
        res = await client.post(f"{BASE}/games/{room_code}/start", headers={"Authorization": f"Bearer {HOST_TOKEN}"})
        if res.status_code not in (200, 201):
            print(f"  FATAL: {res.status_code} {res.text}")
            return
        game_id = res.json()["id"]
        ...
        # Connect host
        ws = await websockets.connect(f"{WS_BASE}?token={HOST_TOKEN}&room_code={room_code}")
        ws_conns["host"] = ws
        ...
        # Connect players
        for p in PLAYERS:
            ws = await websockets.connect(f"{WS_BASE}?token={player_tokens[p['email']]}&room_code={room_code}")
            ws_conns[p["email"]] = ws
            await drain(ws, timeout=3)
        ...
        res = await client.post(f"{BASE}/games/{game_id}/advance-phase", json={"next_phase": "interaction"}, headers={"Authorization": f"Bearer {HOST_TOKEN}"})
        ...
        for ws in ws_conns.values():
            await drain(ws, timeout=1)
        res = await client.post(f"{BASE}/games/{game_id}/advance-phase", json={"next_phase": "discussion"}, headers={"Authorization": f"Bearer {HOST_TOKEN}"})
        ...
        for ws in ws_conns.values():
            await drain(ws, timeout=1)
        res = await client.post(f"{BASE}/games/{game_id}/advance-phase", json={"next_phase": "voting"}, headers={"Authorization": f"Bearer {HOST_TOKEN}"})
        ...
        for p in PLAYERS + [HOST]:
            ...
            ws = ws_conns["host"] if p == HOST else ws_conns[p["email"]]
            name = "Host" if p == HOST else p["username"]
            await ws_send_collect(ws, {"type": "CAST_VOTE", "payload": {"target_user_id": target}})
        ...
        res = await client.post(f"{BASE}/games/{game_id}/advance-phase", json={"next_phase": "result"}, headers={"Authorization": f"Bearer {HOST_TOKEN}"})
        ...
        res = await client.post(f"{BASE}/games/{game_id}/advance-phase", json={"next_phase": "game_over"}, headers={"Authorization": f"Bearer {HOST_TOKEN}"})
