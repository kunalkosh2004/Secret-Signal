    ...
    async with httpx.AsyncClient() as client:
        ...
        for u in users:
            ...
            if res.status_code in (200, 201):
                ...
                check(
                    f"signup {u['username']}", False, f"{res.status_code}: {res.text}"
                )
                continue
        ...
        res = await client.get(
            f"{BASE}/auth/me", headers={"Authorization": f"Bearer {alice_token}"}
        )
        check(
            "GET /auth/me returns current user",
            res.status_code == 200 and res.json()["username"] == "GFT_Alice",
        )
        ...
        check(
            "initial phase is role_assignment", res.json()["phase"] == "role_assignment"
        )
        ...
        if res.status_code == 200:
            ...
            check(
                "game likely completed (advance rejected)",
                res.status_code == 400,
                f"{res.status_code}",
            )
        ...
        async with SL() as db:
            ...
            check(
                "message_sent events exist",
                "message_sent" in event_types,
                f"types={event_types}",
            )
        ...
        async with SL() as db:
            ...
            check(
                ...
                len(training_data) > 0,
                f"count={len(training_data)}",
            )
            if training_data:
                roles = set(t.role for t in training_data)
                check(
                    "training data has multiple roles",
                    len(roles) >= 2,
                    f"roles={roles}",
                )
        ...
        async with SL() as db:
            ...
            check(
                "analysis has players",
                len(analysis.players) == 4,
                f"count={len(analysis.players)}",
            )
            ...
            check(
                "player usernames included",
                all(len(u) > 0 for u in usernames),
            ...
            if coord:
                check(
                    "coordinator has messages",
                    coord.message_count > 0,
                    f"count={coord.message_count}",
                )
                check(
                    "coordinator has username",
                    len(coord.username) > 0,
            ...
            check(
                "summary has no raw user IDs",
                "Player 6" not in analysis.summary
                and "Player 7" not in analysis.summary
        ...
        async with SL() as db:
            ...
            check(
                "4 game players", len(game_players) == 4, f"count={len(game_players)}"
            )
